...
if __name__ == '__main__':
    kpi = 20
    path = '../data/presure/temp/kpi' + str(kpi) + '.csv'
    ...
    print(sum(y_true), sum(y_presure), len(y_true))

    threshold = 1 - (sum(y_true) + sum(y_presure)) / len(y_true)
    seasonal = 1440
    y_pre, y_presure_pre, score, score_nostress = scan_anomaly_poolacoe(data, y_complex, threshold, seasonal, True)
    ...
    _, _, _, _, _, _, _, _, y_presure_pre_eval, text = evaluate.evaluate('name', False, y_presure_pre, y_presure, threshold)
    print('stress', text)
